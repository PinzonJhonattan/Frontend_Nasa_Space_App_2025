...
@Component({
  ...
  imports: [
    ...
    Select, DatePicker, InputGroupModule, 
...
export class MapComponent implements OnInit, OnDestroy {
  ...
  async getWeatherForecast() {
    ...
    try {
      ...
      const eventDate = this.rangeDates[0]; // Usar la primera fecha del rango
      ...
      if (startIndex === -1 || endIndex === -1 || startIndex > endIndex) {
        throw new Error('No se encontró pronóstico para el rango de tiempo seleccionado');
      }
      ...
      this.errorMessage.set(error instanceof Error ? error.message : 'Error al obtener el pronóstico');
    } finally {
